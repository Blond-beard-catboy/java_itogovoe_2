...
public class Main {
    ...
    public static void main(String[] args) {
        try (var sc = new Scanner(System.in)) {
            ...
            try (var anim = TerminalCatAnimation.start(resolveAnimFile(pet))) {
                maxTextWidth = anim.getTextWidthLimit();
    ...
    private static <T> T choose(Scanner sc, String label, List<T> templates, Function<T, String> describer, Supplier<T> customCreator) {
        println("\nЭтап: Выбор " + label);
    ...
    private static void println(String m) { synchronized(System.out) { System.out.println(wrap(m)); } }
    private static void print(String m) { synchronized(System.out) { System.out.print(wrap(m)); System.out.flush(); } }

    private static String wrap(String m) {
        if (m == null || maxTextWidth <= 0) return m;
        StringBuilder sb = new StringBuilder();
        String[] lines = m.split("\n", -1);
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i];
            while (line.length() > maxTextWidth) {
                int cut = line.lastIndexOf(' ', maxTextWidth);
                if (cut == -1) cut = maxTextWidth;
                sb.append(line.substring(0, cut).stripTrailing()).append("\n");
    ...
    private static String resolveAnimFile(Pet p) {
        String s = (p.name() + " " + p.species() + " " + p.feature()).toLowerCase();
